...
<body>
	...
	$sql = "SELECT R.*, C.*, H.* FROM `reservas` R INNER JOIN `clientes` C ON R.id_cliente = C.id INNER JOIN `habitaciones` H on R.nro_habitacion = H.id where R.id = '$pid'";
	...
	while($row = mysqli_fetch_array($re))
	{
		...
		$lname = $row['apellidos'];
		$email = $row['email'];
		$National = $row['nacionalidad'];
		$country = $row['pais'];
		$phone = $row['telef'];
		$room_type = $row['tipo_habitacion'];
		$Bed_type = $row['tipo_cama'];
		//$Noof_room = $row['Nroom'];
		$meal_type = $row['comidas'];
		$cin_date = $row['fecha_entrada'];
		$cout_date = $row['fecha_salida'];
		$nodays = $row['cant_dias'];
	...
	?>
		...
		<article>
			...
			<table class="meta">
				<tr>
					<th><span > ID del Cliente</span></th>
			...
			<table >
					<tr> 
						<td>Teléfono :<?php echo $phone; ?> </td>
						
						<td>Email : <?php echo $email; ?> </td>
					</tr>
					<tr> 
						<td>País : <?php echo $country; ?> </td>
						<td>Nacionalidad : <?php echo $National; ?> </td>
					</tr>
				</table>
				<br>
				<br>
